...
// Helper to convert standards object to MapLibre match expression
function standardsToMapLibreExpression(): any {
  const expression: any = ["match", ["get", "cpt_standard"]];

  Object.entries(standards).forEach(([standard, color]) => {
    expression.push(standard, color);
  });

  // Fallback color (same as "onbekend")
  expression.push(standards.onbekend);
...
const map = new maplibregl.Map({
  ...
  style: {
    ...
    sources: {
      "cpt-netherlands": {
        type: "vector",
        url: "pmtiles://https://r2.bedrock.engineer/cpt_netherlands.pmtiles",
      },
    },
    layers: [
      {
        ...
        paint: {
          "circle-color": standardsToMapLibreExpression(),
          "circle-radius": [
            "interpolate",
            ["exponential", 2],
            ["zoom"],
  ...
  maxZoom: 17,
  maxBounds: [
    [1.5, 49.0], // Southwest corner [lng, lat]
    [8.5, 55.5], // Northeast corner [lng, lat]
  ],
});
...
function updateFilter() {
  if (currentColorMode === "standard") {
    ...
    const filterExpression: Array<any> = ["any"];
    ...
    map.setFilter("cpt-points", filterExpression as any);
  }
}
