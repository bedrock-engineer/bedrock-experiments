...
@app.cell
def _(pyproj):
    lon =4.493600470682538
    lat=51.909737633090394
    transformer = pyproj.Transformer.from_crs(4326, 28992, always_xy=True)
    easting, northing = transformer.transform(lon, lat)
    easting, northing = round(easting, 2), round(northing, 2)
...
@app.cell
def _(trimesh):
    box = trimesh.creation.box(extents=[100, 100, 0.5]).apply_translation([0, 0, 0])
    box
    return (box,)
...
def _(box, np):
    faces = box.faces
    np.column_stack([
        np.full(len(faces), faces.shape[1], dtype=faces.dtype),
        faces
    ]).ravel()
    return
...
@app.cell
def _(geotop_df, mo, pl, trimesh, uuid):
    geotop_trimesh_df = geotop_df.with_columns(
        pl.col("prop.grondsoort").alias("material_name"),
        pl.concat_str(
            [
                pl.lit("FF"),
        ...
        .map_elements(
            lambda c: (
                trimesh.creation.box(extents=[100, 100, 0.5]).apply_translation([c["x"], c["y"], c["z"]])
            ),
    ...
    mo.ui.table(
        geotop_trimesh_df, style_cell=lambda _r, _c, hex: {"backgroundColor": hex}
    )
    return
